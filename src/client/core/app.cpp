...
void App::OnPacketReceived(const NetworkPacket &packet)
{
    ...
    case PacketType::EmitBrowserEvent:
    {
        ...
        if (browserId == -1)
        {
            for (auto const &[id, inst] : browser_.GetAllBrowsers())
            {
                SendEmitToBrowser(browser_, id, eventName, event.args);
            }
            LOG_DEBUG("[CEF] Broadcast event '{}' sent to all browsers", eventName.c_str());
            break;
        }
